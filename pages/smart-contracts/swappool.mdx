...
The sarafu.network swap pool enables users to deposit and exchange (withdraw) community asset vouchers (CAVs), with prices being determined by their relative values against each other.
...
The swap pool has a feature where it will only allow a certain quantity of CAV's and ERC20 to be deposited in.
...
1. Check allowance before hand, if there is a smll allowance, reset it to 0 first before seeking another allowance.
